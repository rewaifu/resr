...
def main(
    ...
    normalize: bool = typer.Option(False, '--normalize', help='Apply median blur and normalization'),
) -> None:
    tiler = ExactTileSize(tile_size)

    os.makedirs(output_folder, exist_ok=True)

    model_paths = list(scandir(model_folder, recursive=True))
    ...
    with tqdm(total=total) as pbar:
        for model_name in model_names:
            if os.path.isabs(model_name):
                ...
                model_path_list = [p for p in model_paths if model_name in os.path.basename(p)]
            ...
            for img_path in image_paths:
                ...
                pbar.set_description(f'Model: {model_name_str} | Image: {img_name}')
                ...
                output_path = os.path.join(output_folder, f'{basename}_{base_model_name}.png')
